...
class TariffServiceError(Exception):
    ...
    problem-type URI); the instance carries the occurrence-specific ``detail`` message. A
    route raises one of these instead of constructing an :class:`~fastapi.HTTPException`
    inline, so the status<->problem mapping lives in exactly one place.
    """
