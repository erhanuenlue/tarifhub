...
in `ai_map`: a single seam fills exclusively the missing, non-billing-relevant fields with
...
change them. The build itself is multi-agent: model-bound workers implement and review, and
an independent second model family (Codex gpt-5.5) reviews every pull request. That second
model caught a metadata write that broke the idempotence of re-import, as well as a
...
around it. An enforced, tested determinism boundary, an AST test in CI that forbids any LLM
client on the value path plus the `guard_frozen` hook that actually stopped a faulty edit below
