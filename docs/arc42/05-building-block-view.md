...
| Serving API (TarifCore) | `services/serving/` | L1 | Read-only REST: list/get tariffs (incl. `?as_of=` point-in-time), version diff, deterministic semantic search (pgvector on Postgres, in-process cosine offline, [ADR-017](../adr/017-deterministic-search-fallback-explain.md)), record-grounded `/api/v1/explain`, FHIR R4 read adapter (ChargeItemDefinition/CodeSystem). The generated OpenAPI document is committed at `services/serving/openapi.json` and drift-tested against `app.openapi()` (see [§8](08-crosscutting-concepts.md)). No write path, no LLM client importable (AST-tested) |
| MCP server (TarifMCP) | `services/mcp/` | L1 | `search_tariffs`, `get_tariff`, `explain_record`: read-only httpx proxies to the serving API, returning frozen records verbatim |
...
| KassenFlow, MeldePilot | `apps/kassenflow/`, `apps/meldepilot/` | L3 (out of platform scope) | Out-of-scope future-work demo stubs ([ADR-013](../adr/013-demo-scope.md)), not part of the graded MVP. Planned as thin read-only Next.js clients over serving (L1) and TarifIQ (L2) that compute no billing value, packaged like every sub-system (Dockerfile + Helm) but shipped `enabled: false` and not yet wired up. Listed here so the building-block view matches what Compose and Helm deploy ([§7](07-deployment-view.md)) |
...
The two L3 apps **KassenFlow** and **MeldePilot** are the same kind of out-of-scope scaffold one layer up: future-work Next.js stubs (payer correspondence / Kostengutsprache and mandatory reporting, respectively) intended to read the deterministic platform like the console but not part of the graded MVP, as their READMEs and [ADR-013](../adr/013-demo-scope.md) state. They are packaged like every other sub-system so Compose and Helm can bring up the full topology, but they ship `enabled: false` in the chart and nothing is wired up. They appear in the container table above only so the building-block view matches what the deployment actually contains ([§7](07-deployment-view.md)).
...
Canonical DDL: the forward-only migrations in `db/migrations/` (`001_init.sql` is the greenfield baseline), with `db/schema.sql` as the consolidated current-state snapshot that CI and the Postgres-enabled parity harnesses apply, mirrored in SQLite for offline tests. The field set is locked additive-only per [ADR-003](../adr/003-canonical-record-model.md). The Pydantic model (`models/tariff_model.py`, `TariffRecord`) is the same shape end-to-end.
