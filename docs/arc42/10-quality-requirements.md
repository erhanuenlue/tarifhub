...
Rows marked **(lands this release)** describe the *target* behaviour for endpoints
implemented in the current branch (point-in-time `?as_of=`, `/diff`, `/api/v1/explain`,
the offline deterministic search fallback on SQLite, and the FHIR R4 read adapter); their
acceptance criteria are written against that target. Rows marked **(design scope)** are
not yet implemented — the criterion records the intended contract per the cited ADR.
...
| **UC-09** Explain (labelled, never a value) | **Given** a frozen code; **When** `GET /api/v1/explain?code=…` is called (directly or via the MCP `explain_crosswalk` tool); **Then** the response carries all frozen versions of the matching records **verbatim** plus a **deterministic, rule-generated** explanation assembled only from record fields and labelled as such (`[deterministic]` prefix) — no LLM on the serving path, **no served billing value altered**; unknown code → HTTP 404. The console's separate AI explain panel keeps its own seam: clearly AI-labelled (`.ai-content`, "AI-generated — not a billing value") and de-identified. | `test_explain_crosswalk_proxies_code` (`services/mcp/tests/test_tools.py`) pins the proxy path `/api/v1/explain`; serving endpoint + explain tests land this release. | partial — console UI + de-id live, serving endpoint **(lands this release)** |
| **FHIR R4 read** (part of UC-04/UC-06) | **Given** a frozen record; **When** the FHIR R4 read adapter serves a `ChargeItemDefinition` (single record) or a `CodeSystem` (a tariff system's codes); **Then** the resource is a valid FHIR R4 representation of the *same* frozen values, computed by a pure read mapping that alters no value. | target: read-only FHIR adapter over the same `repository` reads as REST; value bytes unchanged. | **(lands this release)** |
