...
2. The query is embedded with `get_embedder().embed_query`, the same embedder seam that ingestion used to embed the records (e5's asymmetric `query:`/`passage:` prefixes are honoured).
...
| REST | `GET /api/v1/tariffs/{system}/{code}` | `as_of?` | `TariffRecord` (latest or point-in-time). Returns `404` if no version matches |
| REST | `GET /api/v1/tariffs/{system}/{code}/diff` | `from`, `to` (versions) | `DiffResponse`: changed fields only, sorted by name. `record_hash`, `version`, `created_at` are never diffed. Returns `404` if either version is missing |
| REST | `GET /api/v1/explain` | `code`, `system?` | `ExplainResponse`: every version of the code plus a rule-generated explanation labelled `[deterministic]`. Returns `404` if unknown |
| REST | `GET /api/v1/search` | `q`, `limit` | `list[SearchHit]`: frozen records ranked by cosine similarity. Returns `501` on Postgres when the embedder is not 1024-dim |
| FHIR R4 | `GET /api/v1/fhir/ChargeItemDefinition/{system}/{code}` | `version?` (wins over) `as_of?` | one R4 `ChargeItemDefinition`. `status` is derived from record data, never the wall-clock. Returns `404` if no record matches |
| FHIR R4 | `GET /api/v1/fhir/CodeSystem/{system}` | `as_of?`, `limit`, `offset` | one R4 `CodeSystem` (`content='fragment'`, `count` is the total number of keys for the selected point in time, independent of the page window). Returns `404` for an unknown or empty system |
| MCP | `search_tariffs`, `get_tariff`, `explain_record` | read-only tool arguments | read-only `httpx` proxies onto the REST surface: `get_tariff` returns a frozen record verbatim, `search_tariffs` returns ranked `SearchHit` wrappers, `explain_record` returns the `explain` payload (records plus the deterministic explanation) |
