...
- (+) Durable replay across reconnect and server restart with no missed events, the payoff of C-01 persistence.
...
*Lineage: new. Cross-reference C-01 (the `seq` the stream is keyed on), C-03 (the stream and its control POSTs share the security model).*
