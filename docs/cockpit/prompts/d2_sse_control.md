...
Run at `/effort ultracode`. Read `01-contracts.md` section 4 (the SSE wire contract is authoritative) and ADR-020/021. This is where it becomes a cockpit, not a viewer. Ship as two ordered sub-steps so a partial land cannot merge a broken board (should-fix S12).
...
- Zero new Python dependencies. Vendored HTMX/Alpine JS only (no CDN, no npm), per ADR-020. No em-dashes.
