...
- (+) *Update 2026-06-14: this note corrects the arc42 chapters that described console testing as planned or deferred.* The console `test` script (`vitest run && playwright test`) has been wired since the console landed (PR #16, commit `265b2e3`, 2026-06-13), and the CI `console` job runs it on every push to `main` and every pull request: 40 Vitest tests across six files (asserting the brand visual law: frozen values in navy mono with provenance chips, every AI output on its labelled `.ai-content` surface) plus a Playwright end-to-end smoke. The latest `main` run is 40 Vitest passed and the Playwright smoke passed, with a `CAPTURE`-gated screenshot spec skipped. The demo-scope decision deferred only the ingestion review endpoint above, never the console tests.
