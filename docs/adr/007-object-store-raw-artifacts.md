...
## Decision
We store immutable raw source artifacts in S3-compatible object storage — MinIO in local dev (compose profile), Exoscale SOS in production — and reference each artifact from the append-only `audit_log`.
...
- (–) One more infrastructure component and credential set; the database cannot enforce referential integrity into the bucket — revisit (add a consistency-check job) when the first artifact-reference mismatch surfaces.
