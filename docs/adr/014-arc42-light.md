...
## Decision
We keep arc42 in a light profile — all 12 chapters present, only load-bearing sections filled — combined with C4 diagrams and this ADR register, published via MkDocs Material (print-site for the PDF hand-in).
...
- (–) Some chapters stay thin — acceptable in the light profile; revisit if a grader or reviewer cannot locate a criterion's evidence via the criterion map.
