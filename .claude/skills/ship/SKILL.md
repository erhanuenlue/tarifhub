...
**Effort policy (owner law): maximum reasoning throughout, no down-shift to save tokens** (the Fable-era `/effort ultracode` session command is retired, ADR-018). Every inline phase (plan, gates, fix, PR, report, merge-gate) runs at maximum reasoning. Every delegated worker carries `effort: ultracode` in its own frontmatter (implementer, e2e-tester, verifier, determinism-auditor, security-reviewer, grade-auditor; codex-reviewer relays gpt-5.6-sol on the owner's Codex login). Auto-orchestration operates **inside** this skill's contract: the nine phases and their order stand, the emits fire, **gate 01 always stops for Erhan**, and phase 09 auto-merges only when its green-contract holds, anything less stops for him.
